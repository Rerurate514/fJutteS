...
describe('ProviderObserver', () => {
    ...
    it('巨大なViewオブジェクトを回避できる', () => {
        ...
        expect(updateHistory[0].provider).toBe('provider1');
        expect(updateHistory[0].oldValue).toEqual({ key: 'old value' });
        expect(updateHistory[0].newValue).toBe(`${newValue.constructor.name}__viewId:${newValue.id}`);
    
        expect(logSpy).toHaveBeenCalledWith(
            `Update: provider1 changed from ${JSON.stringify(oldValue)} to "${newValue.constructor.name}__viewId:${newValue.id}"`
        );
